#[cfg(test)]
mod tests {
    use crate::utilities::fs_dkr::refresh_message::RefreshMessage;
    ...
    use crate::utilities::fs_dkr::error::FsDkrResult;
    use curv::{
    ...
    fn test1() {
        ...
        simulate_dkr::<{ crate::utilities::fs_dkr::M_SECURITY }>(&mut keys);
    ...
    fn test_remove_sign_rotate_sign() {
        ...
        let offline_sign = simulate_offline_stage(keys.clone(), &[1, 2, 3]);
        simulate_signing(offline_sign, b"ZenGo");
        simulate_dkr_removal::<{ crate::utilities::fs_dkr::M_SECURITY }>(&mut keys, [1].to_vec());
        let offline_sign = simulate_offline_stage(keys.clone(), &[2, 3, 4]);
        simulate_signing(offline_sign, b"ZenGo");
        simulate_dkr_removal::<{ crate::utilities::fs_dkr::M_SECURITY }>(&mut keys, [1, 2].to_vec());
    ...
    fn test_add_party_with_permute() {
        fn simulate_replace<const M: usize>(
            ...
            let (mut join_messages, new_keys) =
                generate_join_messages_and_keys::<{ crate::utilities::fs_dkr::M_SECURITY }>(party_indices.len());
        ...
        simulate_replace::<{ crate::utilities::fs_dkr::M_SECURITY }>(&mut keys, &[2, 7], &old_to_new_map, t, n)
            .unwrap();
        // check that sum of old keys is equal to sum of new keys
