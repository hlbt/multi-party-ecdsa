...
use crate::utilities::fs_dkr::error::FsDkrError;
...
impl<E: Curve, H: Digest + Clone> RingPedersenStatement<E, H> {
    pub fn generate() -> (Self, RingPedersenWitness<E, H>) {
        let (ek_tilde, dk_tilde) =
            Paillier::keypair_with_modulus_size(crate::utilities::fs_dkr::PAILLIER_KEY_SIZE).keys();
        let one = BigInt::one();
...
mod tests {
    ...
    fn test_ring_pedersen() {
        ...
        let proof = RingPedersenProof::<Secp256k1, Sha256, { crate::utilities::fs_dkr::M_SECURITY }>::prove(
            &witness, &statement,
        );
        assert!(
            RingPedersenProof::<Secp256k1, Sha256, { crate::utilities::fs_dkr::M_SECURITY }>::verify(
                &proof, &statement
            )
            .is_ok()
        );
